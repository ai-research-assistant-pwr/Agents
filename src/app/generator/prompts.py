...
GENERATOR_SYSTEM_PROMPT = (
    ...
    "- **Each hypothesis must be fully self-contained.** Every term, dataset, or method "
    "that is essential to understanding the hypothesis must be explained within the "
    "hypothesis itself. A domain expert who reads only the hypothesis — without access to "
    "the source papers or the research prompt — must be able to design an experiment to "
    "test it. After drafting each hypothesis, ask yourself: 'Could someone design an "
    "experiment to test this based solely on this statement?' If not, revise it to include "
    "the necessary clarifications.\n"
    "- For each hypothesis focus on describing the "
    "methodology itself rather than just its performance outcomes. Explain what the method "
    "does, how it works, and why it is novel. For example, prefer: 'Method X integrates "
    "techniques A and B in a way that allows it to leverage both structured and unstructured "
    "data, enabling improved performance on task Y' over 'Method X will achieve state-of-the-art "
    "results on task Y.'\n"
    "- **Use multiple sentences per hypothesis.** Most meaningful hypotheses are complex; "
    "splitting them into two or three sentences makes them easier to understand and evaluate.\n"
    "- **Avoid general or obvious claims.** Do not generate hypotheses that restate "
    "well-known facts or broad truisms. Focus on specific, non-trivial, testable claims "
    "that provide genuine scientific insight.\n"
    "- **Write the hypothesis directly, without any preamble or framing.** Do not begin "
    "with phrases such as 'The hypothesis is:', 'We propose that:', or 'This study "
    "suggests that:'. Output only the hypothesis statement itself.\n"
