...
Method:
  1. Embed the meaning space (prompt + paper abstracts) and the signal space
     (Retriever output_content from Turn 0 + Turn 2) for every trajectory.
...
Method (offline, no extra inference required):
  - We exploit runs where `apply_channel_noise=True` so that each trajectory
    ...
    (noisy) for turns 0 and 2.
  - We embed Turn-3 generator outputs paired with their respective
    (clean / noisy) inputs and compute the cosine distance between the two
    Generator outputs.  A higher distance indicates the Generator is sensitive
    to signal perturbations, i.e. it *listens*.
...
def _extract_meaning(data: dict) -> str:
    """Build meaning-space string: prompt + top-5 paper abstracts."""
...
def _extract_signal(data: dict) -> str:
    """
    Signal-space string: concatenated clean Retriever outputs (Turn 0 + 2).
    ...
    parts = []
    for turn in turns:
        role = turn.get("role") or turn.get("agent_role", "")
        if role == "retriever":
...
def _extract_noisy_signal(data: dict) -> str | None:
    """
    Noisy signal: concatenated `noisy_output_content` for retriever turns.
    ...
    for turn in turns:
        role = turn.get("role") or turn.get("agent_role", "")
        if role == "retriever" and "noisy_output_content" in turn:
...
def _extract_generator_output(data: dict) -> str:
    """Turn 3 generator output_content."""
    turns = data.get("turns", [])
    for turn in turns:
        if turn.get("turn_id") == 3:
...
async def run_positive_listening(
    ...
    Cross-run (--noisy_logs_dir supplied):
        Compare Generator (Turn 3) output embeddings between a baseline run
