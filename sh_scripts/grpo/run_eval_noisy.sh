#!/bin/bash
# =============================================================================
# run_eval_noisy.sh — Noise Probe: Noisy Condition (noise_probability=1.0)
# =============================================================================
#
# Przeprowadza jeden przebieg eval-only na zbiorze rl_grounded_dataset_eval
# z noise_probability=1.0 i n_samples_per_prompt=10.
# Trajektorie zapisuje do workflow_logs/<PROBE_NAME>/eval_noisy/eval/
#
# Uruchamiaj razem z run_eval_clean.sh (mogą chodzić równolegle jako osobne joby).
#
# Usage:
#   sbatch sh_scripts/grpo/eval/run_eval_noisy.sh <WANDB_API_KEY> <WEAVIATE_NODE_ID>
#
# Env overrides:
#   TRAIN_MODEL      — ścieżka do modelu (default: merged_sft_qwen_4B)
#   PROBE_NAME       — wspólna nazwa folderu z run_eval_clean.sh (default: noise_probe)
...
# Wspólna nazwa z run_eval_clean.sh — muszą być takie same, żeby eval_noise_probe.py
...
# Pełna ścieżka: workflow_logs/<PROBE_NAME>/eval_noisy/eval/traj_*.json
DEBUG_DIR="$BASE_DIR/workflow_logs/$PROBE_NAME/eval_noisy"
OUTPUT_DIR="$BASE_DIR/models_output/${PROBE_NAME}_noisy_results"
...
echo "========================================================"
echo " Noise Probe — NOISY condition"
...
# =============================================================================
# 6. Eval-only pass — NOISY
# =============================================================================
echo ""
echo "=> Uruchamianie eval-only (NOISY, noise=${NOISE_PROBABILITY}, n_samples=${N_SAMPLES})..."
...
echo ""
echo "========================================================"
echo " NOISY eval complete."
echo " Trajektorie: $DEBUG_DIR/eval/"
echo ""
echo " Gdy run_eval_clean.sh również się skończy, uruchom:"
echo "   python src/eval/eval_noise_probe.py \\"
echo "     --noisy_dir $DEBUG_DIR \\"
echo "     --clean_dir $BASE_DIR/workflow_logs/$PROBE_NAME/eval_clean \\"
