...
WANDB_RUN="${WANDB_RUN_NAME:-$SLURM_JOB_NAME}" # Defaults to 'ppo_qwen'
BATCH_SIZE="${TRAIN_BATCH_SIZE:-32}"
ROLLOUT_SIZE="${ROLLOUT_BATCH_SIZE:-32}"
...
EPS_CLIP="${EPS_CLIP:-0.2}"           # PPO clipping range for policy loss
VALUE_CLIP="${VALUE_CLIP:-0.5}"       # PPO clipping range for value loss
CRITIC_LR="${CRITIC_LR:-9e-6}"        # Critic (value network) learning rate
ACTOR_LR="${ACTOR_LR:-5e-7}"          # Actor learning rate
GAE_LAMBD="${GAE_LAMBD:-0.95}"        # GAE lambda for advantage estimation
GAE_GAMMA="${GAE_GAMMA:-1.0}"         # GAE gamma (discount factor)
INIT_KL_COEF="${INIT_KL_COEF:-0.01}" # KL penalty coefficient
...
# PPO requires a critic model; critic_pretrain is set to the same base model
# so MARTI initializes the value head from the policy model weights.
AGENT0="{
    \"0\": {
        ...
        \"critic_pretrain\": \"$TRAIN_MODEL\",
        \"is_tuning\": true,
        \"is_reasoning_model\": true
    }
}"

echo "=> Running MARTI PPO training (Workflow Mode)..."
