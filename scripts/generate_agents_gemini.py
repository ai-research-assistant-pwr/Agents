...
RETRIEVED_CONTEXTS_FILE = os.path.join(LOCAL_BASE, CONFIG['files']['retrieved_contexts'])
...
retriever_prompt = ChatPromptTemplate.from_messages([
    ("system", """You are an Expert Scientific Retriever Agent.
...
generator_prompt = ChatPromptTemplate.from_messages([
    ("system", """You are an AI Research Scientist generating scientific hypotheses.
...
async def process_item(row, semaphore: asyncio.Semaphore, csv_writer, f_csv, f_jsonl):
    async with semaphore:
        ...
        try:
            ...
            retriever_output = await (retriever_prompt | llm_retriever).ainvoke(retriever_inputs)
            ...
            generator_output = await (generator_prompt | llm_generator).ainvoke(generator_inputs)
...
async def main():
    ...
    fieldnames = [
        "prompt_id", "user_query", "raw_context", 
        "retriever_reasoning", "retriever_extracted_info", 
        "generator_is_answerable", "generator_reasoning", 
        "generator_hypothesis", "generator_falsification"
    ]
